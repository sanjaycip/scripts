...
if fs_available "overlayfs" ; then
	...
	mount -t overlay -o lowerdir=${RO_DIR},upperdir=${CACHE_DIR},workdir=${CACHE_DIR}.overlayfs none ${MOUNT_POINT}
elif fs_available "aufs" ; then
	echo "mounting aufs"
	mount -t aufs -o br=${CACHE_DIR}=rw:${RO_DIR}=ro -o udba=reval none ${MOUNT_POINT}
else
	return 1
fi
